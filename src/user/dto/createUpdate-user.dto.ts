...
export class CreateUpdateUserDto{
    ...
    @IsNotEmpty()
    @IsString()
    @Length(MINIMUM_LENGTH,MAXIMUM_LENGTH, { message: 'Username length should be between 8 and 15 characters' })
    username:string

    
    @IsNotEmpty()
    @IsString()
    @Length(MINIMUM_LENGTH,MAXIMUM_LENGTH, { message: 'Password length should be between 8 and 15 characters' })
